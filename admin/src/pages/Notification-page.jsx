...
const badgeColors = {
  ...
  claim_approved:  { bg: "#f0fdf4", color: "#16a34a", border: "#dcfce7" },
  claim_rejected:  { bg: "#fef2f2", color: "#dc2626", border: "#fee2e2" },
  item_matched:    { bg: "#f5f3ff", color: "#7c3aed", border: "#ede9fe" },
    ...
    <div style={{ background: "#fff", border: "1px solid #e8d0d0", borderRadius: 20, padding: 24, boxShadow: "0 2px 12px rgba(74,0,16,0.05)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {filtered.length === 0 ? (
          ...
          filtered.map((item) => {
            const badge = badgeColors[item.type] || badgeColors.message;
            return (
              <div
                key={item.id}
                style={{
